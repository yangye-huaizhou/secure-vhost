...
//虚拟机发包，后端收包，vq[1]
static inline int vdev_dequeue(struct vhost_dev *hdev)
{
...
//虚拟机收包，后端发包，vq[0]
static inline int vdev_enqueue(struct vhost_dev *hdev)
{
	...
	for (pkt_idx = 0; pkt_idx < count; pkt_idx++) {
		if (unlikely(rte_ring_count(hdev->vhost_vring[0]) == 0))
				...
				uint32_t pkt_len = ((struct rte_mbuf *) pkt)->pkt_len + hdev->vhost_hlen;
...
//		uint32_t pkt_len = pkts[pkt_idx]->pkt_len + hdev->vhost_hlen;
		...
		rte_ring_sc_dequeue(hdev->vhost_vring[0],&pkt);
                pkts[pkt_idx]=(struct rte_mbuf*)pkt;
