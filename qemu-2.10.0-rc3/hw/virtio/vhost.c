...
extern struct spdk_ioat_chan *global_ioat;

QLIST_HEAD(, vhost_dev) vhost_devices =
                                ...
                                unsigned idx)
{
    ...
    if (k->query_guest_notifiers &&
        ...
        file.fd = -1;
		//dev->virtio_vq[idx]->callfd=file.fd;
		//printf("callfd=%d\n",file.fd);
                   ...
                   VhostBackendType backend_type, uint32_t busyloop_timeout)
{
    ...
    hdev->dev_ioat = malloc(sizeof(struct spdk_ioat_chan *));
    hdev->dev_ioat = global_ioat;
    hdev->ioat_submit = 0;
...
int vhost_dev_start(struct vhost_dev *hdev, VirtIODevice *vdev)
{
	...
	//vritio_vq=malloc(2*sizeof(struct virtio_virtqueue));
    hdev->started = true;
    hdev->vdev = vdev;
    ...
    /*
    error = pthread_create(&(hdev->ntid[1]), NULL, packet_process_send_burst, NULL);
