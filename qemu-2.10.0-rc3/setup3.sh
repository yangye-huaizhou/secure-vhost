...
if [ "$res" = "0" ]
then
nohup qemu-system-x86_64 -c 13 -w 0000:00:04.0 -- -machine accel=kvm -cpu host -smp sockets=1,cores=1,threads=1 -m 1024M -object memory-backend-file,id=mem,size=1024M,mem-path=/dev/hugepages,share=on -drive file=/var/iso/virtual$temp.qcow2 -mem-prealloc -numa node,memdev=mem -vnc 0.0.0.0:$vncnum --enable-kvm -chardev socket,id=char$temp,path=/tmp/sock$i,server -netdev type=vhost-user,id=mynet$temp,chardev=char$temp,vhostforce -device virtio-net-pci,netdev=mynet$temp,id=net$temp,mac=00:00:00:00:00:$temphex &
else
